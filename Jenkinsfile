    ...
    stage("Deploy to S3") {
      ...
      steps {
        withCredentials([
          ...
          sh "aws s3 cp build/dcos-ui-update-service s3://downloads.mesosphere.io/dcos-ui-update-service/latest/dcos-ui-update-service --metadata-directive REPLACE --cache-control max-age=0,no-cache,no-store,must-revalidate --acl public-read"
        }
      }
    }
  }
